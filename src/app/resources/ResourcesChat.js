...
const ResourcesChat = forwardRef(function ResourcesChat(
  {
    selectedResources = [],
    onClearSelection,
    onRemoveResource,
    onFilterToReferencedResources,
    userDetails,
    allCollections = [],
    allEvents = [],
    allOrganizations = [],
  ...
  useImperativeHandle(ref, () => ({
    ...
    addResourceToContext: (resource) => {
      // Add the resource to mentioned items if it's not already there
      setMentionedItems((prev) => {
        const exists = prev.find((existing) => existing.id === resource.id);
        if (!exists) {
  ...
  // Convert selectedResources to the expected format
  ...
  const selectedAdditionalItems = useMemo(() => {
    ...
    // Add some collections, events, etc. if needed
  ...
  }, [isOpen, currentHistory.length, history.length, typingAnimationEnabled]);
  ...
  const handleSendMessage = async () => {
    ...
    try {
      ...
      // Add context about selected resources
      const contextNote =
        selectedResourcesData.length > 0
          ? `Note: User has selected ${selectedResourcesData.length} resource(s) for analysis.`
          : "Note: User has not selected any specific resources.";

      const matchedChildAttachments = allItemsToSend
        .filter((item) => item.type === "resource")
        .flatMap((item) => item.matchedChildren?.attachments || []);

      const matchedChildLinkGroups = allItemsToSend
        .filter((item) => item.type === "resource")
      ...
      const payload = {
        ...
        data: {
          ...
          videos: [],
          notations: [],
          attachments: allItemsToSend
            .filter((item) => item.type === "attachment")
            .map((item) => item.id)
            .concat(matchedChildAttachments.map((item) => item.id)),
          linkGroups: allItemsToSend
            .filter((item) => item.type === "link_group")
            .map((item) => item.id)
            .concat(matchedChildLinkGroups.map((item) => item.id)),
          mentionedItems:
      ...
      if (history.length === 0) {
        setLocalHistory((prev) => [
          // Disable typing animation on all previous messages
          ...prev.map((msg) => ({ ...msg, enableTypingAnimation: false })),
          aiMessage,
        ]);
      }
          ...
          <div className="flex items-center gap-2 md:gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-gray-600">
                {selectedResourcesData.length} resource
                {selectedResourcesData.length !== 1 ? "s" : ""} selected
              </p>
            </div>
          </div>
          <button
              ...
              <div>
                <div className="flex items-center gap-2 mb-2">
                  ...
                  <span className="text-sm font-medium text-gray-600">
                    Resources ({selectedResourcesData.length})
                  </span>
                </div>
                <div className="ml-4 space-y-1">
                  {selectedResourcesData.slice(0, 3).map((resource) => (
                    <div key={resource.id} className="text-xs text-gray-600">
                      {resource.name || resource.title}
                    </div>
                  ))}
                  {selectedResourcesData.length > 3 && (
                    <div className="text-xs text-gray-500">
                      ... and {selectedResourcesData.length - 3} more
                    </div>
                  )}
                </div>
              </div>
            ...
            {selectedResourcesData.length === 0 &&
              selectedAdditionalItems.length === 0 && (
                <div className="text-xs text-gray-500 italic">
                  No context items selected. Select resources or collections to
          ...
          {currentHistory.length === 0 ? (
            <div className="text-center text-gray-500 mt-4 md:mt-8">
              ...
              <p className="text-xs md:text-sm">
                Ask questions about your selected resources
              </p>
        ...
        <div className="p-3 md:p-4 border-t border-gray-200 bg-white rounded-none md:rounded-b-xl">
          ...
          <div className="relative">
            <textarea
              ...
              placeholder="Ask about resources... (Use @ to mention specific items)"
...
ResourcesChat.displayName = "ResourcesChat";

...
const CollectionChat = ({
  ...
  const [selectedResources, setSelectedResources] = useState({
    collection: true, // Always include the main collection
    description: true,
    resources: {},
    externalLinks: {},
    events: {},
  ...
  const removeMentionedItem = (itemToRemove) => {
    ...
    const mentionText = `@${itemToRemove.title}`;
  ...
  // Utility function to truncate content to prevent payload size issues
  ...
  const estimatePayloadSize = (payload) => {
    try {
      ...
      // Fallback estimation
      return JSON.stringify(payload).length * 2; // Rough estimate
    }
  };
  ...
  const buildChatContext = () => {
    ...
    // Always include basic collection info
    if (selectedResources.collection) {
      context.push({
        type: "collection",
        id: collection.id,
        title: collection.name,
        name: collection.name,
        description: truncateContent(collection.description, 1000),
        type_category: collection.type,
    ...
    // Include selected resources with truncated content
      ...
      collection.resources.forEach((resource) => {
        if (selectedResources.resources[resource.id]) {
          context.push({
            ...
            content: truncateContent(resource.content, 2000), // Limit to 2000 chars
            description: truncateContent(resource.description, 500),
    ...
    // Include selected external links with truncated content
      ...
      collection.externalLinks.forEach((link) => {
        if (selectedResources.externalLinks[link.id]) {
          context.push({
            ...
            name: link.name,
            url: link.url,
            description: truncateContent(link.description, 500),
    ...
    // Include selected attachments from external links with truncated content
    if (collection.externalLinks) {
      collection.externalLinks.forEach((link) => {
        if (link.attachments) {
          link.attachments.forEach((attachment) => {
            if (selectedResources.attachments[attachment.id]) {
              context.push({
                ...
                description: truncateContent(attachment.description, 500),
  ...
  const handleSendMessage = async () => {
    ...
    try {
      ...
      const payload = {
        ...
        // Move disableRAG to top level - when using mentions OR when we have any items
        disableRAG: allItemsToSend.length > 0,
      };

      // Check payload size before sending
      const payloadSize = estimatePayloadSize(payload);
      const maxSize = 95000; // Leave some buffer under 100KB limit

      if (payloadSize > maxSize) {
        toast.error(
          `Message too large (${Math.round(
            payloadSize / 1000
          )}KB). Try selecting fewer resources or use shorter content.`
        );
        setIsLoading(false);
        return;
      }
      ...
      if (error.message && error.message.includes("too large")) {
        toast.error(
          "Message too large. Try selecting fewer resources or use shorter content."
        );
        ...
        <div className="p-3 md:p-4 border-b border-gray-200 bg-gray-50 max-h-32 md:max-h-48 lg:max-h-64 overflow-y-auto">
          ...
          <div className="space-y-1 md:space-y-2">
            ...
            <label className="flex items-center gap-2 text-xs md:text-sm">
              <input
                type="checkbox"
                checked={selectedResources.collection}
                onChange={() => handleResourceToggle("collection")}
            ...
            {collection.resources && collection.resources.length > 0 && (
              <div className="ml-4 space-y-1">
                ...
                {/* {collection.resources.length > 5 && (
                  <div className="text-xs text-gray-500 ml-6">
                    +{collection.resources.length - 5} more resources
                  </div>
                )} */}
              </div>
            )}
            ...
            {collection.externalLinks &&
              collection.externalLinks.length > 0 && (
                <div className="ml-4 space-y-1">
                  <div className="flex items-center justify-between">
                    <span className="text-sm font-medium text-gray-600">
                      External Links ({collection.externalLinks.length})
                    </span>
                    <button
                      onClick={() =>
                        handleSelectAllCategory(
                  ...
                  {/* {collection.externalLinks.length > 3 && (
                    <div className="text-xs text-gray-500 ml-6">
                      +{collection.externalLinks.length - 3} more links
                    </div>
                  )} */}
                </div>
              )}

            {/* Attachments from External Links */}
            {collection.externalLinks &&
              collection.externalLinks.some(
                (link) => link.attachments && link.attachments.length > 0
              ) && (
                <div className="ml-4 space-y-1">
                  <div className="flex items-center justify-between">
                    <span className="text-sm font-medium text-gray-600">
                      Attachments (
                      {collection.externalLinks.reduce(
                        (acc, link) => acc + (link.attachments?.length || 0),
                        0
                      )}
                      )
                    </span>
                    <button
                      onClick={() => {
                        const allAttachments = collection.externalLinks.reduce(
                          (acc, link) => [...acc, ...(link.attachments || [])],
                          []
                        );
                        handleSelectAllCategory(
                          "attachments",
                    ...
                    .map((attachment) => (
                      ...
                      >
                        <input
                          type="checkbox"
                          checked={
                            selectedResources.attachments[attachment.id] ||
                            false
                          }
                          onChange={() =>
                        ...
                        index === history.length - 1 && (
                          <ReferencedItems
                            ...
                            onUseReferencedItems={(formattedItems) => {
                              ...
                              // For CollectionChat, we could update the context or show a toast
                              toast.success(
        ...
        <div className="p-3 md:p-4 border-t border-gray-200 bg-white rounded-none md:rounded-b-xl">
          ...
          <div className="relative">
            <textarea
              ...
              placeholder="Ask a question about this collection... Use @ to mention other items"

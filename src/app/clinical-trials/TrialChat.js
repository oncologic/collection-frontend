...
const TrialChat = forwardRef(function TrialChat(
  {
    selectedTrials = [],
    onClearSelection,
    onRemovePublication,
    onFilterToReferencedTrials,
    userDetails,
    allResources = [],
    allCollections = [],
    allEvents = [],
  ...
  useImperativeHandle(ref, () => ({
    ...
    addTrialToContext: (trial) => {
      // Add the trial to mentioned items if it's not already there
      setMentionedItems((prev) => {
        const exists = prev.find(
          (existing) => existing.id === trial.NCTId || existing.id === trial.id
        );
        if (!exists) {
  ...
  // Convert selectedTrials to the expected format
  const selectedTrialsData = useMemo(() => {
  ...
  const buildChatContext = () => {
    ...
    // Add selected trials
    selectedTrialsData.forEach((trial) => {
      context.push({
        type: "clinical_trial",
        id: trial.NCTId || trial.id,
        title: trial.BriefTitle,
  ...
  const handleSendMessage = async () => {
    ...
    try {
      ...
      // Add context about selected trials
      const contextNote =
        selectedTrialsData.length > 0
          ? `Note: User has selected ${selectedTrialsData.length} clinical trial(s) for analysis.`
          : "Note: User has not selected any specific clinical trials.";
      ...
      const payload = {
        ...
        history,
        data: {
          clinicalTrials: selectedTrialsData,
          collections: [],
          externalLinks: [],
          resources: [],
          events: [],
          ...
          <div className="flex items-center gap-2 md:gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-gray-600">
                {selectedTrialsData.length} trial
                {selectedTrialsData.length !== 1 ? "s" : ""} selected
              </p>
            </div>
          </div>
          <button
            ...
            {selectedTrialsData.length > 0 && (
              <div>
                <div className="flex items-center gap-2 mb-2">
                  <FaFlask className="w-3 h-3 text-orange-600" />
                  <span className="text-sm font-medium text-gray-600">
                    Trials ({selectedTrialsData.length})
                  </span>
                </div>
                <div className="ml-4 space-y-1">
                  {selectedTrialsData.slice(0, 3).map((trial) => (
                    <div key={trial.NCTId} className="text-xs text-gray-600">
                      {trial.BriefTitle}
                    </div>
                  ))}
                  {selectedTrialsData.length > 3 && (
                    <div className="text-xs text-gray-500">
                      ... and {selectedTrialsData.length - 3} more
                    </div>
                  )}
                </div>
              </div>
            ...
            {selectedTrialsData.length === 0 &&
              selectedAdditionalItems.length === 0 && (
                <div className="text-xs text-gray-500 italic">
                  No context items selected. Select trials or resources to
          ...
          {history.length === 0 ? (
            <div className="text-center text-gray-500 mt-4 md:mt-8">
              ...
              <p className="text-xs md:text-sm">
                Ask questions about your selected clinical trials
              </p>
                    ...
                    <div>
                      ...
                      {message.referencedItems &&
                        ...
                        index === history.length - 1 && (
                          <ReferencedItems
                            items={[
                              ...(chatData?.data?.clinicalTrials?.map(
                                (trial) => ({
                                  ...trial,
                                  type: "clinical_trial",
                                })
        ...
        <div className="p-3 md:p-4 border-t border-gray-200 bg-white rounded-none md:rounded-b-xl">
          ...
          <div className="relative">
            <textarea
              ...
              placeholder="Ask about clinical trials... (Use @ to mention specific items)"
...
TrialChat.displayName = "TrialChat";

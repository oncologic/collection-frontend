...
import {
  useGetBusinessUnit,
  useGetBusinessUnitUsers,
  useSubscribeToBusinessUnit,
  useUnsubscribeFromBusinessUnit,
  useUpdateBusinessUnit,
} from "@/app/hooks/useBusinessUnits";
import Modal from "@/app/components/Modal";
import AddBusinessUnitForm from "@/app/components/forms/AddBusinessUnit";
import { useTags } from "@/app/hooks/useTags";
import { useGetResoucesByBusinessUnit } from "@/app/hooks/useResources";
...
import { UNASSIGNED_BUSINESS_UNIT_ID } from "@/app/constants/businessUnits";
...
const BusinessUnitPage = () => {
  const { id } = useParams();
  const isUnassignedBusinessUnit = id === UNASSIGNED_BUSINESS_UNIT_ID;
  ...
  // Data fetching hooks
  const { data: fetchedBusinessUnit, isLoading: businessUnitLoading } = useGetBusinessUnit(
    id,
    {
      enabled: !isUnassignedBusinessUnit,
    }
  );
  const { data: events = [], isLoading: eventsLoading } = useEvents();
  const { data: resources = [], isLoading: resourcesLoading } =
    useGetResoucesByBusinessUnit(id);
  const { data: tags = [] } = useTags();
  const { data: businessUnitSurveys = [] } = useFetchSurveys();
  const { data: users = [], isLoading: usersLoading } =
    useGetBusinessUnitUsers(id, { enabled: !isUnassignedBusinessUnit });
  const { mutate: updateBusinessUnit } = useUpdateBusinessUnit();

  // Fetch associated social media accounts
  const { data: socialMediaAssociations = [], isLoading: socialMediaLoading } =
    useAssociatedSocialMediaAccounts(id, "business_unit", {
      enabled: !isUnassignedBusinessUnit,
    });

  const businessUnit = useMemo(() => {
    if (!isUnassignedBusinessUnit) {
      return fetchedBusinessUnit;
    }

    const resourceCount = resources.length;

    return {
      id: UNASSIGNED_BUSINESS_UNIT_ID,
      name: "Unassigned",
      description:
        resourceCount === 1
          ? "1 resource without an assigned business unit."
          : resourceCount > 1
          ? `${resourceCount} resources without an assigned business unit.`
          : "Resources without an assigned business unit.",
      imageUrl: null,
      tags: [],
      website: null,
      isVirtual: true,
  ...
  }, [fetchedBusinessUnit, isUnassignedBusinessUnit, resources.length]);
  ...
  // Reset image error when businessUnit changes
  useEffect(() => {
    setImageError(false);
  }, [businessUnit?.id, businessUnit?.imageUrl]);

  // Determine if user can edit this businessUnit
  const canEditBusinessUnit = useMemo(() => {
    if (!businessUnit || isUnassignedBusinessUnit) return false;

    // Check if user has edit access to this businessUnit
    return (
      // User is in personal tenant and org is in their personal tenant
      (isPersonal && businessUnit.userId === systemUser?.id) ||
      // User is admin in the businessUnit's tenant
      adminTenants?.some(t => t.tenantId === businessUnit.tenantId) ||
      // User is advocate in the businessUnit's tenant
      advocateTenants?.some(t => t.tenantId === businessUnit.tenantId)
    );
  }, [
    businessUnit,
    isPersonal,
    ...
    advocateTenants,
    systemUser,
    isUnassignedBusinessUnit,
  ]);

  // Filter events for this businessUnit
  ...
  }, [events, id, isUnassignedBusinessUnit]);
  ...
  // Handlers
  const handleUpdateBusinessUnit = async (updatedData) => {
    updateBusinessUnit(
      { id, data: updatedData },
      {
        onSuccess: () => {
          setIsEditing(false);
          queryClient.invalidateQueries(["businessUnit", id]);
        },
      }
    );
  };
  ...
  const isPageLoading = isUnassignedBusinessUnit ? resourcesLoading : businessUnitLoading;
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-blue-50 via-white to-indigo-50">
      <div className="container mx-auto px-4 py-8 w-11/12 max-w-11/12">
        ...
        {(canEditBusinessUnit || (isAdmin && businessUnit.tenantId === systemUser.tenantId)) && (
          <div className="mb-8 flex flex-col-reverse sm:flex-row justify-between items-center gap-4 ">
            {isAdmin && businessUnit.tenantId === systemUser.tenantId && (
              <Tabs
                activeTab={activeView}
                onChange={setActiveView}
                fontSize="text-md"
              >
                <Tab id="details" label="Business Unit Details" />
        ...
        <div className="bg-white rounded-2xl shadow-lg border border-gray-100 mb-8 overflow-hidden">
          <div className="p-8">
            {/* Business Unit Image and Basic Info */}
            <div className="flex flex-col md:flex-row items-center md:items-start gap-6 mb-6">
              <div className="flex-shrink-0">
                {businessUnit.imageUrl && !imageError ? (
                  ...
                  <PlaceholderAvatar name={businessUnit.name} />
                )}
              </div>
              ...
              <div className="flex-1 text-center md:text-left">
                ...
                {/* Tags */}
                {Array.isArray(businessUnit.tags) &&
                  businessUnit.tags.length > 0 && (
                    <div className="flex flex-wrap justify-center md:justify-start gap-2 mb-4">
                      {businessUnit.tags.map((tag) => (
                        <span
                          key={tag.id}
        ...
        {activeView === "details" && (
          <div className="space-y-8">
            ...
            {!isUnassignedBusinessUnit &&
              (filteredEvents.length > 0 || businessUnitSurveys.length > 0) && (
              <Summary
                events={filteredEvents}
                // surveys={businessUnitSurveys}
                resources={[]}
              />
            )}

            {/* Enhanced Resources Section */}
            {(isUnassignedBusinessUnit || (resources && resources.length > 0)) && (
                  ...
                  <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-4">
                    <div>
                      ...
                      <p className="text-gray-600">
                        {isUnassignedBusinessUnit
                          ? "Discover resources that are not assigned to a business unit"
                          : `Discover resources shared by ${businessUnit.name}`}
                      </p>
                    </div>
                    ...
                    <div className="text-center py-12">
                      ...
                      <p className="text-gray-600">
                        {hasActiveResourceFilters
                          ...
                          : "This business unit hasn't shared any resources yet."}
                      </p>
                    </div>
                  ) : (
                    <Summary

...
import { useGetResoucesByBusinessUnit } from "../hooks/useResources";
import LoadingSkeleton from "../components/LoadingSkeleton";
import BusinessUnitCard from "../components/BusinessUnitCard";
import Modal from "../components/Modal";
import AddBusinessUnitForm from "../components/forms/AddBusinessUnit";
import BusinessUnitsList from "../components/business-units/BusinessUnitsList";
...
import { UNASSIGNED_BUSINESS_UNIT_ID } from "../constants/businessUnits";
...
function enhanceOrgs(businessUnits) {
  return businessUnits.map((org, i) => {
    if (org.isVirtual) {
      return org;
    }
...
const SubscriptionsTable = ({ businessUnits, onUnsubscribe, searchTerm }) => {
  const filteredOrgs = businessUnits.filter(
    (org) =>
      ...
      {filteredOrgs.length === 0 && (
        <div className="text-center py-8 text-gray-500 bg-gray-50/50">
          <div className="flex flex-col items-center gap-2">
            ...
            <span>No business units found matching your search.</span>
          </div>
        </div>
      )}
    </div>
...
export default function BusinessUnitsPage() {
  ...
  const { data: rawOrgs = [], isLoading: orgsLoading } = useBusinessUnits();
  const { data: unassignedResources = [] } = useGetResoucesByBusinessUnit(
    UNASSIGNED_BUSINESS_UNIT_ID
  );
  const { data: userSubscriptions = [] } = useUserSubscriptions();

  // Create a map of subscribed business unit IDs for easier lookup
  ...
  const unassignedBusinessUnit = useMemo(() => {
  ...
  const businessUnits = useMemo(() => {
    const enhancedBusinessUnits = enhanceOrgs(rawOrgs);

    return unassignedBusinessUnit
      ? [unassignedBusinessUnit, ...enhancedBusinessUnits]
      : enhancedBusinessUnits;
  }, [rawOrgs, unassignedBusinessUnit]);
  ...
  // Helper function to count business units by tag
  const getTagBusinessUnitCount = (tagId, filteredOrgs) => {
    return (
  ...
  const filteredOrgs = useMemo(() => {
    return businessUnits.filter((org) => {
  ...
  }, [businessUnits, searchTerm, selectedTags]);
  ...
  const createOrg = useCreateBusinessUnit();

  const handleAddBusinessUnit = (org) => {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-gray-50 via-white to-gray-50">
      ...
      <div className="mt-20 z-20 bg-white/95 backdrop-blur-sm border-b border-gray-200 shadow-sm">
            ...
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-gray-600 text-sm mt-1">
                  Discover and subscribe to business units in your field
                </p>
              </div>
            ...
            <div className="flex flex-col sm:flex-row gap-3 lg:flex-1 lg:max-w-2xl">
              <div className="relative flex-1">
                ...
                <input
                  ...
                  placeholder="Search business units, descriptions, or focus areas..."
                />
              </div>
        ...
        <div className="flex-1 p-6 md:p-4">
          <div className="w-11/12 mx-auto">
            ...
            <div className="mb-12">
              <div className="flex items-center justify-between mb-6">
                <div>
                  ...
                  <p className="text-gray-600 text-sm mt-1">
                    Business Units you have added to your favorites
                  </p>
                </div>
                <button
                      ...
                      <div className="flex flex-col items-center gap-4">
                        ...
                        <div>
                          ...
                          <p className="text-gray-600">
                            Add business units to your favorites to keep up with
            ...
            <div>
              <div className="mb-6">
                <h2 className="text-xl font-semibold text-gray-900">
                  Discover Business Units
                </h2>
                <p className="text-gray-600 text-sm mt-1">
                  Find new business units and professional associations to add to
                  your favorites
                </p>
                {(searchTerm || selectedTags.length > 0) && (
                  <p className="text-sm text-blue-600 mt-2">
                    Showing {allBusinessUnits.length} filtered result
                    {allBusinessUnits.length !== 1 ? "s" : ""}
                  </p>
                )}
              </div>
              ...
              <BusinessUnitsList
                ...
                loading={orgsLoading}
                onToggleFavorite={(orgId) => {
                  const org = allBusinessUnits.find((o) => o.id === orgId);
                  if (org) {
              ...
              {!orgsLoading && allBusinessUnits.length === 0 && (
      ...
      {isModalOpen && (
        ...
        >
          <AddBusinessUnitForm
            onSubmit={handleAddBusinessUnit}
            onClose={() => setIsModalOpen(false)}
            tags={tags}
          />
        </Modal>
            ...
            <div className="flex justify-between items-center mb-6">
              <div>
                ...
                <p className="text-gray-600 mt-1">
                  Manage your business unit favorites
                </p>
              </div>
            </div>
